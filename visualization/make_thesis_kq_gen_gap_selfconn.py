...
EXP = "experiments/v2/self_connections/res_metrics"
...
# store[type][self_connection] = {"kq": [...], "gen": [...]}
store = {}
for i, cfg in enumerate(configs):
    wr = cfg.conf["esn"]["W_args"]["W_res_args"]
    typ = wr["type"]
    sc = float(wr["self_connection"])
    m = run_res_metrics(cfg.conf)
    store.setdefault(typ, {}).setdefault(sc, {"kq": [], "gen": []})
    store[typ][sc]["kq"].append(float(m["kernel_quality"]))
    store[typ][sc]["gen"].append(float(m["generalization"]))
    cfg.conf.get("esn", {}).pop("model", None)
    if (i + 1) % 22 == 0:
        print(f"  {i+1}/{len(configs)} done")

# type -> (KQ/Gen color, gap color)
...
for typ in sorted(store.keys(), key=str):   # baseline-esn, constant
    ...
    for s, k, g, d in zip(xs, kq_m, gen_m, gap_m):
        print(f"  sc={s:.1f}  KQ={k:6.1f}  Gen={g:6.1f}  KQ-Gen={d:6.1f}")

plt.xlabel("self_connection")
plt.ylabel("Score")
plt.title("KQ vs Gen by type")
plt.legend(fontsize="small")
plt.grid(True, alpha=0.3)
plt.tight_layout()
out = "/tmp/type_vs_self_connection_kq_gen_gap_lines.png"
